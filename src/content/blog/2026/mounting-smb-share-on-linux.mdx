...
For Debian-based systems, this is the package that you need:

```sh
$ sudo apt install cifs-utils
```

I haven't checked for others, so YMMV.
...
Fill out the details like below - rather than mounting the whole shared folder I
map a specific subfolder on the share to this directory on the client:
...
By default the files in this shared directory will be `755` (`-rwxr-xr-x`) on
the client. If you want to allow non-root users to be able to write to the share
you can add this to the `Options` line:
...
If you need to change the contents of the service, ensure you reload the systemd
daemon with the latest definition:
