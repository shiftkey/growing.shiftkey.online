...
I'll do a separate writeup of the onboarding problems that I had during setup (it was 3-ish years ago and I don't think I have the artifacts from that testing, so I'd need to use the actual camera again) but the two main hurdles I had at the time:
...
It took me a bit to realize that `OV5647` is the camera module identifier, and while I was able to view an image from the camera without specifying any settings it had a red tint to it. In this case I needed to update my script to use the `noir` tuning file via code:
